...
export function renderLuxuryPropertyWebsite(input = {}) {
  ...
  const title = `${data.brand} - ${data.mode === 'property' ? 'Private Property' : 'Private Collection'}`;
  ...
  return `<header class="navbar lux-navbar" id="luxuryHeader">
    ...
    <div class="navbar-end gap-2">
      ...
      <details class="dropdown dropdown-end lux-mobile-nav">
        <summary class="btn btn-circle btn-ghost" aria-label="Open navigation"><span aria-hidden="true">Menu</span></summary>
...
export function renderLuxuryMediaHero(data) {
  ...
  return `<section id="top" class="template-full-bleed-hero lux-hero relative overflow-hidden bg-neutral text-white">
    ...
    <div class="lux-shell lux-hero-content relative z-10">
      ...
      <div class="lux-hero-foot"><span>${escapeHtml(data.locationLine)}</span><a href="#practice">Discover the practice</a></div>
    </div>
  </section>
  ...
  return `<section class="lux-section lux-contact" id="contact">
    <div class="lux-shell lux-contact-grid">
      ...
      <form class="lux-form reveal" id="luxuryEnquiryForm">
        ...
        <label><span>Area of interest</span><select class="select select-bordered" name="interest">${data.services.map(item => `<option>${escapeHtml(item.title)}</option>`).join('')}<option>Other private enquiry</option></select></label>
        <label><span>How may we assist?</span><textarea class="textarea textarea-bordered" name="message" rows="5"></textarea></label>
        <div class="lux-form-foot"><p>Your details will be used only to respond to this enquiry.</p><button class="btn btn-neutral" type="submit">Send enquiry <span aria-hidden="true">&rarr;</span></button></div>
        <p class="alert alert-success lux-form-success" id="luxuryFormSuccess" role="status">Thank you. Your enquiry has been received.</p>
      </form>
    </div>
  </section>`;
}
...
function normalizeData(input) {
  ...
  return {
    ...
    paletteId: slug(input.paletteId || ''),
    fontGroupId: slug(input.fontGroupId || ''),
    ...
    introEyebrow: property ? 'The practice' : 'Our perspective',
    ...
    assetsEyebrow: property ? 'Selected property' : 'Selected work',
    assetsHeading: property ? 'Places of consequence.' : 'Objects and experiences of consequence.',
    assetsIntro: property
    ...
    servicesEyebrow: property ? 'Property services' : 'Private services',
    servicesHeading: property ? 'One relationship. Complete alignment.' : 'Considered service from first idea to final detail.',
    servicesIntro: property
    ...
    cinematicStatement: property ? 'The finest properties are not simply found. They are understood.' : 'Luxury is not abundance. It is precision.',
    cinematicEyebrow: 'Point of view',
    cinematicCopy: property
    ...
    practiceImageAlt: property ? 'Refined residential interior' : 'Refined luxury detail',
    principlesEyebrow: 'Independent by design',
    principlesHeading: property ? 'Quietly connected. Entirely client-side.' : 'Selective, attentive, and built around the client.',
    principlesIntro: property
    ...
    audienceEyebrow: property ? 'Who we advise' : 'Who we serve',
    audienceHeading: property ? 'Built around principals, not listings.' : 'Built around lasting relationships, not transactions.',
    ...
    processHeading: 'Measured. Personal. Decisive when required.',
    processIntro: 'Each mandate is shaped around the client while preserving a clear rhythm: alignment, informed decisions, disciplined delivery, and continuity.',
    ...
    privateEyebrow: property ? 'Private view' : 'Private access',
    privateHeading: property ? 'Selected opportunities, shared in confidence.' : 'A more personal view, available by request.',
    ...
    contactEyebrow: 'Private enquiry',
    contactHeading: 'Begin with a conversation.',
    contactIntro: 'Share only what is useful at this stage. A senior member of the team will respond personally and discreetly.',
    contactResponse: 'Personal response within one business day',
    footerLine: property ? 'Discretion - Independence - Judgement' : 'Craft - Service - Continuity',
    images
  };
}
